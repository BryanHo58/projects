...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    ...
    if (!PyObject_TypeCheck(args, &Matrix61cType)){
      PyErr_SetString(PyExc_TypeError, "Addition not supported with other types");
      return NULL;
    }
    Matrix61c *mat2 =  (Matrix61c *) args;
    int rows1 = self->mat->rows;
    int rows2 = mat2->mat->rows;
    int cols1 = self->mat->cols;
    int cols2 = mat2->mat->cols;
    if ((rows1 != rows2) || (cols1 != cols2)){
      PyErr_SetString(PyExc_TypeError, "Dimension mismatch");
      return NULL;
    }
    matrix *result_mat;
    if (allocate_matrix(&result_mat, rows1, cols1)){
    ...
    result->shape = PyTuple_Pack(2, PyLong_FromLong(rows1), PyLong_FromLong(cols1));
    if (add_matrix(((Matrix61c *) result)->mat, self->mat, mat2->mat) != 0) {
      PyErr_SetString(PyExc_RuntimeError, "Allocation failed");
      return NULL;
    }
...
static PyObject *Matrix61c_sub(Matrix61c* self, PyObject* args) {
    /* TODO: YOUR CODE HERE */
    if (!PyObject_TypeCheck(args, &Matrix61cType)){
      PyErr_SetString(PyExc_TypeError, "Subtraction not supported with other types");
      return NULL;
    }
    Matrix61c *mat2 =  (Matrix61c *) args;
    int rows1 = self->mat->rows;
    ...
    result->mat = result_mat;
    result->shape = PyTuple_Pack(2, PyLong_FromLong(rows1), PyLong_FromLong(cols1));
    if (sub_matrix(((Matrix61c *) result)->mat, self->mat, mat2->mat) != 0) {
      PyErr_SetString(PyExc_RuntimeError, "Allocation failed");
      return NULL;
    }
    return (PyObject *) result;
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
    /* TODO: YOUR CODE HERE */
    if (!PyObject_TypeCheck(args, &Matrix61cType)){
      PyErr_SetString(PyExc_TypeError, "Multiplication not supported with other types");
      return NULL;
    }
    Matrix61c *mat2 =  (Matrix61c *) args;
    int rows1 = self->mat->rows;
    ...
    matrix *result_mat;
    if (allocate_matrix(&result_mat, rows1, cols2)){
      PyErr_SetString(PyExc_RuntimeError, "Allocation failed");
      return NULL;
    }
    Matrix61c* result = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    result->mat = result_mat;
    result->shape = PyTuple_Pack(2, PyLong_FromLong(rows1), PyLong_FromLong(cols2));
    if (mul_matrix(((Matrix61c *) result)->mat, self->mat, mat2->mat) != 0) {
      PyErr_SetString(PyExc_RuntimeError, "Allocation failed");
      return NULL;
    }
    return (PyObject *) result;
...
static PyObject *Matrix61c_neg(Matrix61c* self) {
    ...
    result->mat = result_mat;
    result->shape = PyTuple_Pack(2, PyLong_FromLong(rows1), PyLong_FromLong(cols1));
    if (neg_matrix(((Matrix61c *) result)->mat, self->mat) != 0) {
      PyErr_SetString(PyExc_RuntimeError, "Allocation failed");
      return NULL;
    }
    return (PyObject *) result;
...
static PyObject *Matrix61c_abs(Matrix61c *self) {
    ...
    result->mat = result_mat;
    result->shape = PyTuple_Pack(2, PyLong_FromLong(rows1), PyLong_FromLong(cols1));
    if (abs_matrix(((Matrix61c *) result)->mat, self->mat) != 0) {
      PyErr_SetString(PyExc_RuntimeError, "Allocation failed");
      return NULL;
    }
    return (PyObject *) result;
...
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    ...
    int cols1 = self->mat->cols;
    if (cols1 != rows1){
      PyErr_SetString(PyExc_TypeError, "Matrix not square!");
      return NULL;
    }
    if (!PyLong_Check(pow)) {
    ...
    result->mat = result_mat;
    result->shape = PyTuple_Pack(2, PyLong_FromLong(rows1), PyLong_FromLong(cols1));
    if (pow_matrix(((Matrix61c *) result)->mat, self->mat, power) != 0) {
      PyErr_SetString(PyExc_RuntimeError, "Allocation failed");
      return NULL;
    }
    return (PyObject *) result;
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject* args) {
    ...
    if (!PyTuple_Check(args) || (PyTuple_Size(args) != 3)) {
      PyErr_SetString(PyExc_TypeError, "Incorrect Arg formating");
      return NULL;
    }
    if (!PyLong_Check(PyTuple_GetItem(args, 0)) || !PyLong_Check(PyTuple_GetItem(args, 1))
    || (!PyFloat_Check(PyTuple_GetItem(args, 2)) && !PyLong_Check(PyTuple_GetItem(args, 2)))) {
      PyErr_SetString(PyExc_TypeError, "Incorrect arg formating");
      return NULL;
    }
    int row = (int) PyLong_AsLong(PyTuple_GetItem(args, 0));
    int col = (int) PyLong_AsLong(PyTuple_GetItem(args, 1));
    ...
    if ((col >= self->mat->cols) || (col < 0)) {
      PyErr_SetString(PyExc_IndexError, "Column out of range");
      return NULL;
    }
...
static PyMethodDef Matrix61c_methods[] = {
    /* TODO: YOUR CODE HERE */
    {"power", (PyCFunction)Matrix61c_pow, METH_VARARGS,
    ...
    "Gets the value at location (row, col)"},
    {NULL, NULL, 0, NULL},
};
