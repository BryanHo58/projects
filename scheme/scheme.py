...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(procedure, MacroProcedure):
            ...
            expr = rest.map(lambda expr: scheme_eval(expr, env))
...
def eval_all(expressions, env):
    ...
    while subexpression.second is not nil:
        scheme_eval(subexpression.first, env)
        subexpression = subexpression.second
    return scheme_eval(subexpression.first, env, True)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('formals and values do not correspond')
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('incorrect arguments for function')
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        Lambda = do_lambda_form(expressions, env)
        name = target.first
