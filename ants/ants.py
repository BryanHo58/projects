...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if not isinstance(insect, QueenAnt) or insect.fake == True:
            if insect.is_ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        if isinstance(self.max_range, int):
            max_place = place
            for _ in range(self.max_range):
                max_place = max_place.entrance
        while len(place.bees) == 0 and place.entrance != hive and place != max_place:
            place = place.entrance
        ...
        if self.fake:
            ...
            def double_damage(ant):
                if isinstance(ant, BodyguardAnt):
                    if ant.ant is not None and ant.ant not in QueenAnt.doubled_ants:
