...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("auto\\s+([a-z]+)$", this::setAuto),
        new Command("manual\\s+([a-z]+)$", this::setManual),
        new Command("null$", this::doNull)
    };
    ...
    /** Command "null" on USED. */
    private void doNull(Matcher used) {
    }

    /** Command "auto" on USED. */
    private void setAuto(Matcher used) {
        String player = used.group(1).toUpperCase();
        if (player.equals("WHITE")) {
            _white = _autoPlayerTemplate.create(WHITE, this);
        } else if (player.equals("BLACK")) {
            _black = _autoPlayerTemplate.create(BLACK, this);
        } else {
            this.reportError("Invalid command");
        }
    }

    /** Command "auto" on USED. */
    private void setManual(Matcher used) {
        String player = used.group(1).toUpperCase();
        if (player.equals("WHITE")) {
            _white = _manualPlayerTemplate.create(WHITE, this);
        } else if (player.equals("BLACK")) {
    ...
    /** Command "move" on USED. */
    private void doMove(Matcher used) {
        Move test = Move.mv(used.group());
        if (!board().isLegal(test)) {
            reportError("Invalid move. "
                    + "Please try again.");
        } else {
            _board.makeMove(test);
